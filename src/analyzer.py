"""
Main legislation analyzer with enhanced dual action inference, chunking support, and whole document analysis.
CORRECTED VERSION with proper role/category inference using prompting strategies.
"""
import json
import logging
...
class LegislationAnalyzer:
    ...
    async def _apply_comprehensive_document_analysis(self, legislation_text: str, existing_context: str = "", level: str = "level_1", chunk_info: str = "") -> str:
        ...
        messages = [
            SystemMessage(content="You are a legal text analyst. Analyze the ENTIRE document comprehensively. Use simple, clear English."),
    ...
    async def _process_text_chunk_with_context(
        ...
        """Process a single text chunk with full document context."""
        ...
        # Step 4: Synthesize into rules with DUAL actions and full context
        rules = await self._synthesize_rules_with_dual_actions_and_context(
    ...
    def _validate_and_fix_rule_data(self, rule_data: dict, article_reference: str, source_files: dict, applicable_countries: List[str], adequacy_countries: List[str], document_level: str, chunk_reference: Optional[str]) -> dict:
        ...
        # Ensure actions and user_actions are lists
